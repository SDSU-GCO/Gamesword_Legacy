...
namespace UnityStandardAssets.Cameras
{
    ...
    public class AutoCam : PivotBasedCameraRig
    {
        ...
        [SerializeField] private float autoRotateThreshold = 0.1f; // move speed at which camera will auto adjust

        public float xSpeed = 120.0f;
        public float ySpeed = 120.0f;

        public float yMinLimit = -10f;
        public float yMaxLimit = 35f;
        public bool lockedOn = false; // Target is in lock on mode
        ...
        protected override void FollowTarget(float deltaTime)
        {
            ...
            x = Input.GetAxis("Look X") * xSpeed * 0.02f;
            y = Input.GetAxis("Look Y") * ySpeed * 0.02f;
            if (lockedOn)
            {
                transform.rotation = m_Target.rotation;
            }
            else if (targetMoving() && Mathf.Abs(x) < 0.1f && Mathf.Abs(y) < 0.1f) // Automatic rotation
            {
            ...
            else if (Mathf.Abs(x) >= 0.1f || Mathf.Abs(y) >= 0.1f) // Manual Rotation
            {
                Vector3 currentRotation = transform.rotation.eulerAngles;
                if (currentRotation.x > yMaxLimit + 0.5f) // changes angle to negative representation
                    currentRotation.x -= 360f;
                float newX = Mathf.Clamp(currentRotation.x - y, yMinLimit, yMaxLimit);  // Keep angle within limits
                Quaternion rotation = Quaternion.Euler(newX, currentRotation.y + x, 0);
        ...
        bool targetMoving()
        {
            if (Mathf.Abs(targetRigidbody.velocity.x) > autoRotateThreshold || Mathf.Abs(targetRigidbody.velocity.z) > autoRotateThreshold)
                return true;
            else
                return false;
        }
